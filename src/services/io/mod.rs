...
pub fn write_to_json(data: &Vec<models::Item>, file_path: &str) {
    let json_data = serde_json::to_string_pretty(data).expect("JSONへのSiriアライズに失敗しました。");
    let mut file = File::create(file_path).expect("書き込みのファイルのオープンに失敗しました。");
    writeln!(file, "{}", json_data).expect("ファイルへの書き込みい失敗しました。");
